...
# --- FETCH MASTER LIST & 4-TIER NSE REGISTRY ---
...
try:
    ...
    if nse_res.status_code == 200:
        ...
        for _, r in nse_csv.iterrows():
            # Target Tier 4: The absolute most granular Basic Industry classification
...
def get_robust_industry(symbol):
    if symbol in nse_map:
        return nse_map[symbol]
    
    # Absolute fallback for brand new unmapped IPOs or BSE-only transfers
    clean_sym = symbol.replace("-EQ", "").upper()
    ...
    for attempt in range(3):
        try:
            ...
            if hist_data and hist_data.get('data'):
                ...
                df_temp['Industry'] = get_robust_industry(stock['symbol'])
