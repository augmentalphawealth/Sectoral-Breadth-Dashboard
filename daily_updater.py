...
if new_listings:
    print(f"🆕 Syncing {len(new_listings)} new listings...")
    nse_map = {}
    try:
    ...
    for sym in new_listings:
        ind = nse_map.get(sym, "Emerging Equities")
        new_rows.append({"Symbol": sym, "Industry": ind})
...
# --- PRICE DATA FETCH ---
today_dt = pd.to_datetime(datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=5, 30))).strftime("%Y-%m-%d"))
...
print("Calculating EMAs and Metrics...")
...
with open("last_sync.txt", "w") as f:
    f.write(datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=5, 30))).strftime('%d %b %Y, %I:%M %p IST (EOD Sync)'))
print("✅ EOD Run Complete.")
