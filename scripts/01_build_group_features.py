...
def add_stock_indicators(df: pd.DataFrame, settings: dict) -> pd.DataFrame:
    ...
    for win in [20, 50, 150, 200]:
        df[f'sma_{win}'] = g['close'].transform(lambda s: s.rolling(win, min_periods=max(5, win//4)).mean())
    ...
    df['breakout_55'] = ((df['close'] > g['high'].transform(lambda s: s.shift(1).rolling(settings['analysis']['breakout_lookback'], min_periods=20).max())) & (df['volume'] > settings['analysis']['accumulation_volume_multiplier'] * df['avg_vol_20'])).astype(int)
    df['acc_day'] = ((df['close'] > g['close'].shift(1)) & (df['volume'] > settings['analysis']['accumulation_volume_multiplier'] * df['avg_vol_20'])).astype(int)
    df['dist_day'] = ((df['close'] < g['close'].shift(1)) & (df['volume'] > settings['analysis']['distribution_volume_multiplier'] * df['avg_vol_20'])).astype(int)
    ...
    df['vcp_ready'] = ((df['range_20'] <= settings['analysis']['vcp_range_contraction_threshold']) & (df['volume'] <= settings['analysis']['vcp_volume_dryup_threshold'] * df['avg_vol_20']) & (df['dist_52w_high'] > -settings['analysis']['pivot_proximity_threshold'])).astype(int)
    return df
...
def add_group_scores(df: pd.DataFrame, settings: dict) -> pd.DataFrame:
    ...
    df['breadth_score'] = (((df['pct_above_20'] + df['pct_above_50'] + df['pct_above_200']) / 300) * w['breadth'])
    df['rs_score'] = ((df['eq_ret_20d'].rank(pct=True) + df['eq_ret_60d'].rank(pct=True)) / 2) * w['rs']
    df['volume_score'] = ((df['acc_minus_dist'].rank(pct=True)) * w['volume'])
    df['breakout_score'] = ((df['breakout_count'].rank(pct=True) + df['vcp_ready_count'].rank(pct=True)) / 2) * w['breakout']
    df['penalty_score'] = (((df['median_dist_52w_high'] < -0.20).astype(int) * 0.4) + ((df['pct_above_20'] > 85).astype(int) * 0.6)) * w['penalty']
    df['strength_score'] = df['trend_score'] + df['breadth_score'] + df['rs_score'] + df['volume_score'] + df['breakout_score'] - df['penalty_score']

    conditions = [
        (df['strength_score'] >= 70) & (df['pct_above_50'] >= 60),
        (df['strength_score'] >= 55) & (df['pct_above_20'] >= 55),
        (df['pct_above_20'] < 40) & (df['eq_ret_20d'] < 0),
        (df['pct_above_20'] > 80) & (df['median_dist_52w_high'] > -0.05),
    ]
    labels = ['Strong', 'Emerging', 'Weakening', 'Exhausted']
    df['regime'] = np.select(conditions, labels, default='Bottoming')
    return df
...
def main():
    ...
    master = read_parquet_safe(processed / 'security_master_with_kite.parquet')
    prices = read_parquet_safe(processed / 'prices.parquet')

    required = ['symbol', 'date', 'open', 'high', 'low', 'close', 'volume']
    for c in required:
        if c not in prices.columns:
            raise ValueError(f'Missing required prices column: {c}')
    if 'turnover' not in prices.columns:
        prices['turnover'] = prices['close'] * prices['volume']

    join_cols = ['symbol', 'isin', 'industry', 'basic_industry', 'sector', 'series']
    if 'mcap' in master.columns:
        join_cols.append('mcap')
    stock = prices.merge(master[join_cols].drop_duplicates('symbol'), on='symbol', how='left')
