...
def style_with_heatmap(raw: pd.DataFrame, display: pd.DataFrame):
    ...
    if "Buy Priority Score" in raw.columns and "Buy Priority Score" in display.columns:
        styles.loc[:, "Buy Priority Score"] = raw["Buy Priority Score"].map(heat_color)
...
def format_stock_table(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in ["Rank", "Heavy Volume Days (6M)"]:
        if column in data.columns:
            data[column] = data[column].apply(fmt_int)
    for column in [
        "Strength", "Close", "Buy Priority Score", "Vol Contraction (vs 50D)",
...
def top_buy_setups_view(basic_history: pd.DataFrame, stock_history: pd.DataFrame, selected_date: pd.Timestamp) -> None:
    ...
    eligible_df = basic_latest[
        ...
        & (basic_latest.get("leadership_score", 0) >= 70.0)
    ...
    buy_candidates = stock_latest[
        ...
        & (stock_latest.get("established_buy_setup", 0) == 1)
    ].copy()

    ipo_candidates = stock_latest[
        (stock_latest["basic_industry"].isin(top_leaders))
        & (stock_latest.get("ipo_buy_setup", 0) == 1)
    ].copy()
    ...
    m_col[2].metric("Leading Basic Industries", fmt_int(len(top_leaders)))
    ...
    if buy_candidates.empty:
        ...
        # Generate the proxy buy priority score visually if backend didn't save it
        if "buy_priority_score" not in buy_candidates.columns:
            buy_candidates["buy_priority_score"] = 0.0  # Placeholder fallback
        ...
        keep_cols = [
            "Rank", "Symbol", "Basic Industry", "Buy Priority Score", "Close",
    ...
    if ipo_candidates.empty:
        ...
        # Safe `.get()` approach prevents KeyError if pipeline hasn't synced
        ...
        keep_ipo = [
            "Rank", "Symbol", "Basic Industry", "Close",
