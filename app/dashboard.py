# app/dashboard.py
# Complete replacement based on dashboard_final_linked_industry.py.
# Final UI changes:
# - Date control uses a fixed compact width instead of a wide ratio column.
# - Industry navigation is a small inline link below the industry name.
# - Clicking the link sets the constituent industry and automatically scrolls
#   the page to the stock list using a fragment anchor.
...
st.set_page_config(page_title="NSE Industry Momentum Monitor", page_icon="◈", layout="wide", initial_sidebar_state="collapsed")
...
st.markdown(
    ...
    .industry-link {font-size:.74rem; color:#1D4ED8; text-decoration:none; cursor:pointer;}
...
def numeric_column(frame: pd.DataFrame, column: str, default: float = 0.0) -> pd.Series:
    if column not in frame.columns: return pd.Series(default, index=frame.index, dtype=float)
...
def format_number(value: object, decimals: int = 1) -> str:
    try: return "—" if value is None or pd.isna(value) else f"{float(value):,.{decimals}f}"
...
def format_integer(value: object) -> str:
    try: return "—" if value is None or pd.isna(value) else f"{int(round(float(value))):,}"
    except (TypeError, ValueError): return "—"
...
def format_percent(value: object) -> str:
    try:
        ...
        number = float(value)
        if abs(number) <= 1.5: number *= 100.0
        return f"{number:,.1f}%"
    except (TypeError, ValueError): return "—"
...
def score_color(value: object) -> str:
    ...
    except (TypeError, ValueError): return MUTED
...
def change_color(value: object) -> str:
    ...
    return GREEN if number > .05 else RED if number < -.05 else MUTED


def change_indicator(value: object) -> str:
    try: number = float(value)
    except (TypeError, ValueError): return "⚪ —"
    if number > .05: return f"🟢 {format_signed(number)}"
    if number < -.05: return f"🔴 {format_signed(number)}"
...
def leadership_status(score: object, change: object) -> tuple[str, str, str]:
    ...
    if change_value > 0: return "Improving · Not yet confirmed", AMBER, LIGHT_AMBER
    return "Weak leadership", RED, LIGHT_RED


def apply_chart_style(figure: go.Figure, height: int) -> go.Figure:
    figure.update_layout(height=height, margin=dict(l=8, r=25, t=45, b=20), font=dict(family="Inter, -apple-system, Segoe UI, sans-serif", size=12, color=INK), title_font=dict(size=14, color=INK), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", showlegend=False, hoverlabel=dict(bgcolor="white", font_color=INK))
...
def prepare_group_data(frame: pd.DataFrame, group_column: str) -> pd.DataFrame:
    ...
    if "leadership_score" not in data.columns: data["leadership_score"] = data.get("strength_score", 0.0)
...
def prepare_stock_data(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for flag in ["established_buy_setup", "ipo_buy_setup"]:
        if flag not in data.columns: data[flag] = 0
...
def get_trading_dates(frame: pd.DataFrame) -> list[pd.Timestamp]:
    return [] if "date" not in frame.columns else sorted(pd.Timestamp(value) for value in frame["date"].dropna().unique())


def format_sync_time(raw_text: str) -> str:
    text = raw_text.strip()
    if not text or text.lower() == "not available": return "Not available"
    try:
    ...
    except (TypeError, ValueError): return text.replace("T", " ").replace("Z", "")
...
def resolve_trading_date(requested: object, all_dates: list[pd.Timestamp]) -> pd.Timestamp:
    ...
    if requested_date in all_dates: return requested_date
...
def global_date_picker(all_dates: list[pd.Timestamp]) -> pd.Timestamp:
    state_key, widget_key = "global_analysis_date", "global_analysis_date_calendar"
    latest = all_dates[-1]
    ...
    # Fixed compact widths: the calendar no longer expands across the page.
...
def show_table(data: pd.DataFrame, height: int, chart_links: bool = False) -> None:
    ...
    if chart_links and "Chart" in view.columns: config["Chart"] = st.column_config.LinkColumn("Chart", display_text="Open ↗")
...
def current_snapshot(history: pd.DataFrame, selected_date: pd.Timestamp, group_column: str) -> pd.DataFrame:
    ...
    if "members" in data.columns: data = data[numeric_column(data, "members") >= SMALL_GROUP_LIMIT].copy()
...
def priority_score(frame: pd.DataFrame) -> pd.Series:
    return .65 * frame["Leadership Score"] + .35 * frame["5D Leadership Change"].clip(lower=0)
...
def select_industry_everywhere(industry: str) -> None:
    ...
    st.session_state["scroll_to_constituents"] = True
    st.rerun()


def render_industry_link(industry: str, rank: int) -> None:
    # Small, low-profile link below the industry card/name.
    if st.button("↗ constituents + chart", key=f"industry_link_{rank}_{industry}", type="tertiary"):
...
def render_improver_cards(current: pd.DataFrame) -> None:
    ...
    st.caption("Click the small link below an industry name to open its constituent stocks and trend chart.")
...
def render_basic_industry_constituents(stock_history: pd.DataFrame, selected_date: pd.Timestamp, current: pd.DataFrame) -> None:
    ...
    for column in ["20D Return", "60D Return", "6M Gain"]:
        if column in view.columns: view[column] = view[column].map(format_percent)
    for column in ["Close", "Strength"]:
        if column in view.columns: view[column] = view[column].map(lambda value: format_number(value, 1))
...
def render_selected_industry(basic_history: pd.DataFrame, selected_date: pd.Timestamp, current: pd.DataFrame) -> None:
    ...
    if st.session_state.get("selected_basic_industry_trend") not in options: st.session_state["selected_basic_industry_trend"] = options[0]
    ...
    for threshold, color in [(70, DARK_GREEN), (60, GREEN), (50, AMBER)]: figure.add_hline(y=threshold, line_dash="dot", line_color=color, opacity=.85)
...
def prior_move_series(data: pd.DataFrame) -> pd.Series:
    for column in ["gain_6m", "ret_60d", "ret_20d", "ret_120d"]:
        if column in data.columns: return numeric_column(data, column)
...
def render_stock_table(stocks: pd.DataFrame, kind: str, limit: int) -> None:
    ...
    if kind == "IPO" and "ipo_setup_score" in data.columns and (numeric_column(data, "ipo_setup_score") > 0).any(): data["Priority Score"] = numeric_column(data, "ipo_setup_score")
    ...
    view = view[[column for column in keep if column in view.columns]]
    if "Priority Score" in view.columns: view["Priority Score"] = view["Priority Score"].map(lambda value: format_number(value, 1))
...
def methodology_tab() -> None:
    ...
    The small `↗ constituents + chart` link below an industry card updates both the Basic Industry constituents list and the Selected Industry Trend chart, then scrolls the page to the constituent section.

    ## Leadership Score and change
    Leadership Score is a 0–100 relative leadership measure. Five-session change is current score minus the score five available sessions earlier. Read change together with current score.

    ## Top setups
    Established and IPO tables show Priority Score, Tightness (3D), Volume vs 50D and Prior Move. Values are percentages with one decimal and no traffic-light colours.

    ## Risk
    This is a research tool, not a trade recommendation. Verify charts, liquidity, market conditions, entry, invalidation and position sizing independently.
    """)
...
def main() -> None:
    ...
    with tabs[0]: industry_monitor_tab(basic_history, stock_history, selected_date)
    with tabs[1]: stock_setups_tab(stock_history, selected_date)
    with tabs[2]: methodology_tab()
